  ...
  plugins: [
    ...
    VitePWA({
      ...
      workbox: {
        ...
        // and a fallback would risk serving a cached shell. This also fully
        // resolves the black-screen-on-login problem that main_buildout's
        // navigateFallbackDenylist (/auth, /realms, …) previously worked
        // around: with no fallback the SW can never hijack the Keycloak
        // redirect, so the denylist is no longer needed.
